...
class LessonList extends Component {
  ...
  search(e) {
    ...
    for (var i = 0; i < lessons.length; i++) {
      let lesson_title = lessons[i].title.toLowerCase(),
        lesson_tags = lessons[i].tags;
      if (lesson_title.match(val)) filtered.push(lessons[i]);
      else {
        for (var j = 0; j < lesson_tags.length; j++) {
          if (val.match(lesson_tags[j].toLowerCase())) break;
        }
